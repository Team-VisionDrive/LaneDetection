...
import numpy as np
'''
요약      : ROI & Canny & CalcDist & PubX
흐름      : Sub → ROI → Bird-eye View → CannyEdge(윤곽선 검출) → CalcDist → PubX(화면에 출력)
'''
...
class ROI:
    ...
    def image_topic_callback(self, img):
        '''
            ...
            5. 최종 검출된 값을 기반으로 카메라 좌표계 기준 차선 무게중심 점의 x좌표 Publish
        ...
        self.distance_pub.publish(self.distance) #5
